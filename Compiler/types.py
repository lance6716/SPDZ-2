...
class Matrix(MultiArray):
    ...
    def T(self):
        res = [None] * self.sizes[1] * self.sizes[0]
        for i in range(self.sizes[0]):
            for j in range(self.sizes[1]):
                res[j * self.sizes[0] + i] = self[i][j]
        #res_matrix = Matrix(self.sizes[1], self.sizes[0], self.value_type)
        res_matrix = Matrix(self.sizes[1], self.sizes[0], type(res[0]))
        res_matrix.array.assign(res)
        return res_matrix
